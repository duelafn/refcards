...
\begin{block}
\lr{if <EXPR> \{ \} elsif <EXPR> \{ \} else \{ \}}
\lr{unless <EXPR> \{ \}}{no else allowed}
...
\lr{leg, lt, le, eq, ge, gt, ne}{string comparison}
\lr{+, -, *, /, \%, **}{math operators}
\lr{\$<a> mod \$<b>, \$<a> lcm \$<b>, \$<a> gcd \$<b>}{}
...
\begin{code}
  ...
  multi sub fib(0) \{ 1 \}
  multi sub fib(1) \{ 1 \}
  multi sub fib($a) \{ fib($a-1) + fib($a-2) \}
\end{code}
\end{block}
...
\begin{tabularx}{\hsize}{@{}p{1ex}p{2.2cm}X@{}}
  ...
  N & Structural infix & \Code{but does {\<}={\>} leg cmp .. ..{\^} {\^}..}\\
...
utf8 IO Mu Any Cool Junction
Supply Whatever Match Nil Parcel Capture Signature Pair Range Set Bag Mix
