...
\begin{titlesection}
  {\large\bfseries Perl 6.0.0 Reference Guide}\\
  Version 0.6, Dec 2015 --- Dean Serenevy\\
...
\lr{{\~} <\$x>, + <\$x>, ? <\$x>}{string, numeric, boolean coersion}
...
\lr{Nil, True, False}{undefined value, true, false}
...
\lr{@a[5] = 42; @a.elems}{autovivification, num elements}
...
\lr{Q:w[ ], qw[ ], {\<} <x y z> {\>}}{word quoting}
\lr{Q:qq:w[ ], qqw[ ], \string« \$<x> <y z> \string»}{interp.~word quoting}
...
\lr{for <LIST> \{ \}}{implicit \Code{\$_} arg}
...
\begin{code}
  try \{\comment{# try is optional, can CATCH in any block}
   ...
   \comment{# CATCH may appear anywhere within block}
   \comment{# is in same scope so vars are accessible}
   CATCH \{
   ...
   \}
   LEAVE \{
     \comment{# a general phaser, but worth special mention}
     \comment{# Like “finally” in other languages}
   \}
   ...
...
\lr{1..5, 1..\^6, 0\^..\^6, \^6}{each is range 1,2,3,4,5}
\end{block}

\section{I/O}
\begin{block}
\lr{say \"Hello\"; print \"Hello\";}{output with/out newline}
...
\lr{is export, is export(:<foo>)}{exportable from a module. Special tags ":ALL", ":DEFAULT", ":MANDATORY"}
...
\lr{Bool, Bit}
\lr{Int, Rat, FatRat, UInt, Num, Complex}
\lr{Str, Blob, Buf}
...
\section{Processes}
\begin{block}
\lr{shell(\$<cmd>), run(*@<args>)}{command with/out shell return Proc::Status objects}
...
\begin{code}
  ...
  class «Foo» is «MyParent» does «MyRole» \{
    ...
    \comment{# Override for positional (not best practices!)}
    method new(\$x, *@w) \{
